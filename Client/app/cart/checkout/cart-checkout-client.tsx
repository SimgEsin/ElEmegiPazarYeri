...
const deliveryContactsStorageKey = "cart:delivery-contacts"
...
export function CartCheckoutClient({ initialItems }: CartCheckoutClientProps) {
  ...
  useEffect(() => {
    ...
    try {
      const parsed: unknown = JSON.parse(storedCheckoutItems)
      if (!Array.isArray(parsed)) {
        throw new Error("Kayıtlı sepet verisi dizi formatında değil.")
      }

      const parsedItems = parsed.filter(isValidCartLineItem)
      if (parsedItems.length > 0) {
        setItems(parsedItems)
      }
    } catch (error) {
      console.error("Checkout sepet verisi okunamadı:", error)
  ...
  const grandTotal = subtotal + (selectedShipping?.fee ?? 0)
  const isCheckoutReady = Boolean(items.length > 0 && selectedDeliveryContact && selectedShipping && selectedPayment)
    ...
    return (
      <section className="space-y-6 rounded-2xl border border-primary/10 bg-card p-8 shadow-sm">
        ...
        <p className="text-sm text-muted-foreground">
          Sipariş oluşturmak için sepette ürün bulunmalı.
        </p>
    ...
    <section className="space-y-6 rounded-2xl border border-primary/10 bg-card p-8 shadow-sm">
      ...
      <div className="space-y-3">
        ...
        <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground">
          Sepetteki ürünler için aşağıdaki adımları tamamlayarak sipariş oluşturabilirsiniz.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <aside className="space-y-4 rounded-xl border border-primary/10 bg-white p-5 xl:sticky xl:top-6 xl:h-fit">
          ...
          <dl className="space-y-3 text-xs">
            ...
            <div className="space-y-1">
              <dt className="font-semibold text-muted-foreground">Ürün Ara Toplamı</dt>
